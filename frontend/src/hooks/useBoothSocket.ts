...
const BACKEND_WS_URL =
  (import.meta as any).env.VITE_BACKEND_WS_URL || "http://localhost:3000";
...
const SESSION_PRICE = parseFloat(
  (import.meta as any).env.VITE_SESSION_PRICE || "15.0",
);
...
export function useBoothSocket(boothId: string = BOOTH_ID) {
  ...
  const switchPayment = useCallback(
    async (paymentType: "pix" | "card") => {
      try {
        ...
        console.error(err);
        setError(
          "Não foi possível trocar o método de pagamento. Tente novamente.",
        );
      }
    },
    [boothId],
